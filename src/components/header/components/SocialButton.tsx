...
export default function SocialButton({ className, imgSrc, title }: SocialButtonProps) {
  return (
    <button
    ...
    >
      <span className='h-9 w-9 flex items-center justify-center mr-4 bg-white rounded-sm'>
        <img src={imgSrc} alt='google-logo' className='h-[20px] w-[20px] object-fill' height={20} width={20} />
      </span>
      {title}
    </button>
  )
